...
parser.add_argument('--epoch', type=int, default=250, help='Epoch to run [default: 50]')
...
os.system('cp train.py %s\n' % output_dir + '/')
os.system('cp pointnet_part_seg.py %s\n' % output_dir + '/')
...
def placeholder_inputs():
    ...
    return pointclouds_ph, input_label_ph, labels_ph, seg_ph, voxel_ph
...
def train():
    with tf.Graph().as_default():
        with tf.device('/gpu:'+str(FLAGS.gpu)):
            pointclouds_ph, input_label_ph, labels_ph, seg_ph, voxel_ph = placeholder_inputs()
            ...
            labels_pred, seg_pred, end_points = model.get_model(
                    pointclouds_ph, 
                    input_label_ph, 
                    voxel_ph,
...
#                cur_train_filename = os.path.join(hdf5_data_dir, train_file_list[train_file_idx[i]])
                ...
                cur_data, cur_labels, cur_seg, cur_voxel = provider.loadDataFile_with_seg_voxel(cur_train_filename)
                cur_data, cur_labels, order = provider.shuffle_data(cur_data, np.squeeze(cur_labels))
                cur_seg = cur_seg[order, ...]
                cur_voxel = cur_voxel[order, ...]
                #cur_voxel_finer = cur_voxel_finer[order, ...]

                cur_labels_one_hot = convert_label_to_one_hot(cur_labels.astype(np.int32))
                ...
                for j in range(num_batch):
                    ...
                    feed_dict = {
                            ...
                            voxel_ph : cur_voxel[begidx : endidx, ...],
